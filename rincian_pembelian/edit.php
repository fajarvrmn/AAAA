...
<html lang="en">
      ...
      <div class="row">
        <div class="panel panel-primary">
          ...
          <table class="table" >
                      ...
                      <h1><b>Tambah Data Rincian Pembelian</b></h1>
  </center>
  <form action="editpro.php" method="post">
      ...
      <div class="form-group">
        ...
        <select name="no_pembelian"  class="form-control">
                  <?php include('koneksi.php');
                      $mahasiswa=mysqli_query($koneksi,"SELECT * FROM pembelian");
                      foreach ($mahasiswa as $data) 
                      {
                   ?>
                      <option value="<?php echo $data['no_pembelian']?>"><?php echo $data['tgl_pembelian']?> </option>                                 
                  <?php } ?>
                   </select>
      </div>
  ...
  $karyawan          =mysqli_query($koneksi,"SELECT * FROM rincian_pembelian  WHERE no_rincian='$no_rincian' ");
  $data              =mysqli_fetch_array($karyawan)
  ?>
    <div>
      <label> <input type="hidden" name="no_rincian" value="<?php echo $data['no_rincian'];?>"> </label>
    </div>
    <div class="form-group">
        <label>Jumlah :</label>
