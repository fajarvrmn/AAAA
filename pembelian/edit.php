...
<html lang="en">
      ...
      <div class="row">
        <div class="panel panel-primary">
          ...
          <table class="table" >
                      ...
                      <h1><b>Tambah Data Pembelian</b></h1>
  </center>
  <form action="editpro.php" method="post">
      ...
      <div class="form-group">
        ...
        <select name="id_supplier"  class="form-control">
                  <?php include('koneksi.php');
                      $mahasiswa=mysqli_query($koneksi,"SELECT * FROM supplier");
                      foreach ($mahasiswa as $data) 
                      {
                   ?>
  ...
  $karyawan          =mysqli_query($koneksi,"SELECT * FROM pembelian  WHERE no_pembelian='$no_pembelian' ");
  $data              =mysqli_fetch_array($karyawan)
  ?>
    <div>
      <label> <input type="hidden" name="no_pembelian" value="<?php echo $data['no_pembelian'];?>"> </label>
    </div>
    <div class="form-group">
        <label>Tanggal Pembelian :</label>
        <input type="text" name="tgl_pembelian" class="form-control" value="<?php echo $data['tgl_pembelian'];?>" required>
    </div>
     <div class="form-group">
        <label>Total Biaya :</label>
        <input type="text" name="total_biaya" class="form-control" value="<?php echo $data['total_biaya'];?>" required>
    </div>
